...
parser.add_argument('--fl_method', type=str, default='tree', help="name of federated learning method")
...
parser.add_argument('--rounds', type=int, default=100, help="rounds of communication")
...
## set g groups, each group has m clients, g*m = K
group_1 = np.random.choice(range(args.no_clients), 10, replace=False)
group_2 = np.random.choice(list(set(range(args.no_clients)) - set(group_1)), 10, replace=False)
group_3 = list(set(range(args.no_clients)) - set(group_1) - set(group_2))

clients_trn_data_1 = {i: clients_total[i] for i in group_1}
clients_name_1 = list(clients_trn_data_1.keys())
weight_scalling_factor_list_1 = [weight_scalling_factor(clients_trn_data_1, client_name) for client_name in clients_name_1]

clients_trn_data_2 = {i: clients_total[i] for i in group_2}
clients_name_2 = list(clients_trn_data_2.keys())
weight_scalling_factor_list_2 = [weight_scalling_factor(clients_trn_data_2, client_name) for client_name in clients_name_2]

clients_trn_data_3 = {i: clients_total[i] for i in group_3}
clients_name_3 = list(clients_trn_data_3.keys())
weight_scalling_factor_list_3 = [weight_scalling_factor(clients_trn_data_3, client_name) for client_name in clients_name_3]
...
for iter in range(args.rounds):
    ...
    for idx in group_1:
        ...
        w_locals_1.append(copy.deepcopy(w))

    # update edge weights by weight average
    data_transfer_volume = sum([sys.getsizeof(w) for w in w_locals_1])
    data_transfer_volume_per_round.append(data_transfer_volume)
    w_edge_1 = avg(w_locals_1, weight_scalling_factor_list_1)


    for idx in group_2:
        local = LocalUpdate(args, dataset=clients_total[idx])
        w = local.train(net=copy.deepcopy(net_glob).to(args.device))
        w_locals_2.append(copy.deepcopy(w))

    # update edge weights by weight average
    data_transfer_volume = sum([sys.getsizeof(w) for w in w_locals_2])
    data_transfer_volume_per_round.append(data_transfer_volume)
    w_edge_2 = avg(w_locals_2, weight_scalling_factor_list_2)


    for idx in group_3:
        local = LocalUpdate(args, dataset=clients_total[idx])
        w = local.train(net=copy.deepcopy(net_glob).to(args.device))
        w_locals_3.append(copy.deepcopy(w))

    # update edge weights by weight average
    data_transfer_volume = sum([sys.getsizeof(w) for w in w_locals_3])
    data_transfer_volume_per_round.append(data_transfer_volume)
    w_edge_3 = avg(w_locals_3, weight_scalling_factor_list_3)

    data_amount_volume_round = sum(data_transfer_volume_per_round)
    data_transfer_volume_list.append(data_amount_volume_round)

    # average all edge model
    data_amount_1 = sum([len(clients_trn_data_1[client_name][0]) for client_name in clients_name_1])
    data_amount_2 = sum([len(clients_trn_data_2[client_name][0]) for client_name in clients_name_2])
    data_amount_3 = sum([len(clients_trn_data_3[client_name][0]) for client_name in clients_name_3])
    weight_edge_1 = data_amount_1 / (data_amount_1 + data_amount_2 + data_amount_3)
    weight_edge_2 = data_amount_2 / (data_amount_1 + data_amount_2 + data_amount_3)
    weight_edge_3 = data_amount_3 / (data_amount_1 + data_amount_2 + data_amount_3)
    
    w_glob = {}
    for k in w_edge_1.keys():
        w_glob[k] = w_edge_1[k] * weight_edge_1 + w_edge_2[k] * weight_edge_2 + w_edge_3[k] * weight_edge_3

    # copy weight to net_glob
    net_glob.load_state_dict(w_glob)

    # evaluate on test set
    accuracy, precision, recall, f1 = test_img(net_glob, dataset_test, args)
    ...
    if accuracy >= 60:
        print('Round {:3d}, Test accuracy {:.2f}'.format(iter, accuracy))
...
result = {
    ...
    'energy_cost': energy_cost_list
}

with open('result_tree_30_3.pkl', 'wb') as f:
