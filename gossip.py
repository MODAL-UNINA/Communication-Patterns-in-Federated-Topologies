...
parser.add_argument('--fl_method', type=str, default='gossip', help="name of federated learning method")

parser.add_argument('--no_clients', default=30, help="number of clients: K")
parser.add_argument('--num_clients', default=10, help="number of aggregation")
...
for iter in range(args.rounds):
    ...
    for i in range(args.no_clients):
        ...
        idxs = np.random.choice([j for j in range(args.no_clients) if j != i], args.num_clients, replace=False)

        # use the selected clients to update Ci
        new_w = avg([w_locals[i]] + [w_locals[j] for j in idxs], [1/(args.num_clients+1)] * (args.num_clients+1))
...
with open('result_gossip_{}_{}.pkl'.format(args.no_clients, args.num_clients), 'wb') as f:
