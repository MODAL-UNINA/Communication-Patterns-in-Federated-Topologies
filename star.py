...
parser.add_argument('--fl_method', type=str, default='star', help="name of federated learning method")

parser.add_argument('--no_clients', default=50, help="number of clients: K")
parser.add_argument('--num_clients', default=50, help="number of aggregation")
...
for iter in range(args.rounds):
    ...
    idxs_clients = np.random.choice(range(args.no_clients), args.num_clients, replace=False)
    ...
    data_transfer_volume = sum([sys.getsizeof(w) for w in w_locals])
    data_transfer_volume_list.append(data_transfer_volume)

    # update global weights by weight average
    w_glob = avg(w_locals, weight_scalling_factor_list)
    ...
    # record how many rounds need to reach the target accuracy 80%
    if accuracy >= 60:
...
with open('result_star_{}_{}.pkl'.format(args.no_clients, args.num_clients), 'wb') as f:
